...
* It includes all needed utilities to prepare an the needed partitions prior to an installation in Manual partitioning mode.
...
Alternatively the user registered during installation and other members of thhe 'whgeel' group can type:
----
sudo <command>
---
...
* To display or not the applications specific to a desktop in other desktops shoc-desktop or hide(desktop (as regular user)
...
The first regular user created during installation will have speech and braille already enabled in the installed system, if speech was used and braille requested during installation. Other users will need to check or make additional settings, as indicated below.
...
login-chooser
----
and choose one of the login modes: text or lightdm
...
Slint allows includes several graphical environments: BlackBox, Fluxbox, FVWM, LXQt, MATE, ratpoiso,  TWM, and WindowMaker. It is a matter of preference which one you choose.

LXQt and MATE are are full featured desktops, FVWM and WindowMake offer unique features and can be selected with the graphical lightdm login as well and also started from a console typing "startx" the other ones mainly window managers that cna only be started from a console.. All allow you to
access your documents and applications, generally opened in a window.

The default graphical environment can be changed typing as regular user `session-chooser`. In Graphical mode you also can choose amng FVWM, LXWT, MATE and WindowMaker when logging in.
...
<System <|Display boot menu<|This tool give a preview of the boot menu as it will be dipmayed afte booting.
|====
...
The reguar user registered during installatio and other users members of the 'wheel' group can also type commands resreved to root prependig the name of the command by 'sudo' like this for instance: +
*sudo update-grub"
...
====
+
Assuming that you installed Slint64-16.0 the output should be:
+
*Slint 15.0*
+
...
* Run "grub-emu" or "list_boot_entries" (inthe latter case type 'q' the 'n' to quit, not Alt+F4)
