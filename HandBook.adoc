...
* In Auto partitiong mode a disk drive of size at least 50G
* On Manual partitioning modea partition of type Linux of size at least 50G, an EFI system partition and a BIOS boot partition on the same disk drive.
* Sint can be installed on these kinds of drives: hard drives, SSD, NVME, eMMC, USB sticks, SD preferably cards in USB enclosures.
* RAM: at least 2G
...
In addition the Manual installation mode require on the same drive as the root system partition:

* A partition if type BIOS boot partition, of size at least 3M and not formatted.
* A partition of type ESP (EFI system partition) located on the same drive as with at least 32M free space in it, formatted with
...
Insert the installation media (DVD or USB stick) and reboot your machine. To hemp blind users a tune is heard when the boot menu is displayed.
...
In Auto mode the installer proposes to encrypt the root system partiton. If you agree, at each boot the GRUB boot loader will ask you the passphrase
...
This chapter is intended for users needing a screen reader but not acquaibted with Speakup.
...
Slint allows includes several graphical environments: BlackBox, Fluxbox, LXQt and  MATE, TWM, and WindowMaker. It is a matter of preference which one you choose.

LXQt and MATE and XFCE are are full featured desktops, the other ones mainly
