...
Two installation modes are proposed: Automatic (the installer prepare the drive chosen by the user and install Slint on it) and Manual (The installer format the Linux partition chosen by the, fomrat it and install Slint on it). The installer includes a documentation.
...
A wide assortment of software for servers, desktops, and laptops is installed in all Slint systems, which include two graphical desktop environments: MATE and LXQt  XFCE and lightweight window managers. MATE and KXQt provide easy access to all applications and system operations, with auto-mounting of removable devices, plus notifications of available updates of installed software, immediate Internet access, stopping, rebooting, and hibernating the system.
...
* A disk drive of size at least 50G for installation in Auto mode or a partition of type Linux of size at least 50G on a disk drive. All kinds of drives can be used: : hard drives, SSD, NVME, eMMC, USB sticks, SD preferably cards in USB enclosures.
* RAM: at least 2G
...
* It includes all needed utilities to prepare an the needed partitions prior to an installation in Manual partitining mpde.
* The Auto mode of installation instead just needs user to answering questions and provide a contextual help about them.
* If installed alone on a removable device connected through USB, Slint is portable, i.e. usable on any computer able to boot off an USB drive beyond the computer used to install it. For instance if installing on an USB stick you can just plug te stick on a compiter to run it.
...
==== Prepare partitions for Slint

In the Auto installation mode you dedicate a device for Slint, that the installer will partition for you. 

If you intend to use the Manual installation mode you can create partitions for Slint, either before or during installation, or just reuse existing partitions.

If you need to create partitions and are not accustomed to Linux you will probably find easier to do it before installation. We suggest to use http://gparted.org/index.php[gparted] to do that. Gparted can shrink existing partitions to make room, as well as create new partitions in the freed space.  If you do not have a system that supports Gparted (Microsoft Windows) you can use http://gparted.org/livecd.php[Gparted Live].
...
Installation in Manual partitioning mode needs to have on the same drive:

* A partition of type Linux, size at least 50G.
* A partition of type "EFI System", size at least 32M.
* A partition of type BIOS Boot, size 3M, not formatted.

Rationale: This layout allows to boot Slint in Legacy as in EFI modes and also to make Slint portable

=== Slint Installation

<<Overview_of_Slint_Installation,Overview of Slint installation>> +
<<Usage_of_the_installer,Usage of the installer>> +
<<Manual_preparation,Manual preparation of the partitions used to install Slint>> +
<<Encryption,Slint with an encrypted root partition>> +
<<Speakup,Key bindings for the speakup screen reader>> +
<<Glossary,Glossary of technical terms used in the documentation>> +
<<first_steps_after_installation,First steps after installation>>

[[Overview_of_Slint_Installation]]
==== Overview of Slint installation

Slint can be installed on 64bit computer only
...
automatic or manual creation of the partitions used by Slint.

You are proposed to choose between manual or automatic preparation
of the drive Slint will be installed.
...
or xfs. Read 'Manual' to know more. 
...
In case of btrfs sub-volumes will created for home and / and with files
compressed. 'Copy on write' will ease making snapshots and possibly revert a
...
The installer first lets you choose the language to use during installation and
select your keyboard layout, then allows you to type commands.

You read the screen and type commands in a "virtual terminal". The installer
...
The fourth virtual terminal or tty4 displays messages that can provide
information for debugging, otherwise it is not used.

The installer has several modes of interaction with you, the user:
1) You type commands at the prompt and read their output.
2) The installer asks a question, you type the answer and confirm it by
pressing Enter.
3) The installer displays a menu of choices or options: you select one of them
using the up and down arrow keys, then confirm your choice by pressing Enter,
or cancel by pressing Escape.
4) The installer displays information in a pager, as it is doing now.
...
[[Manual_preparation]]
==== Manual preparation of the partitions used to install Slint
...
then the type of file system to create in it among: btrfs, ext4, xfs.

The installer lists for selection only partitions that meet the
requirements stated below:

Size: at least 50G
Type: Linux (can also be named Linux filesystem), not encrypted. This
partition may be formatted or not, but in any case its content will be
wiped out and a new file system generated by the installer.
If the drive in which is the partition is located has a GPT (GUID partition
table) a BIOS boot partition, needed by GRUB in some cases, of size at least
3M and not formatted.
A partition of type ESP (EFI system partition) located on the same drive as
the root system partition, with at least 32M free space in it, formatted with
a 'fat' (or 'vfat') file system as as required by the UEFI specification
...
The blkid and lsblk applications displays information about block devices and
partitions.
...
The installer proposes to encrypt the root partition of Slint

If you accept, at each boot the GRUB boot loader will ask you the passphrase
...
Having an encrypted partition prevents the steal of data it contains in case
of loss or theft of the machine, or of a removable drive.

But this won't protect you if the computer stays running and unattended, only
...
definitively locked and all the data it contains will be lost!

If you really need more space, you will need to backup all the files that you
...
don't make it part of a RAID array and generally write to it: all data in the
Slint system partition would be irrecoverably lost!

To avoid weak passphrases the installer requires that the passphrase include:
1) at least 8 characters.
2) Only non accented lower case and capital letters, digits from 0 to 9,
space and following punctuation characters:
 ' ! " # $ %  & ( ) * + , - . / : ; < = > ? @ [ \ ] ^ _ ` { | } ~
This guarantees that even a new keyboard will have all characters needed to
type the passphrase.
3) At least one digit, one lower case letter, one capital letter and one
punctuation character.

GRUB assumes that "us" keyboard is in use when type the passphrase.
...
[[Glossary]]
==== Glossary of technical terms used in the documentation
...
manager (GRUB in Slint's case). Starting the OS can be done two ways or modes:
In the Legacy or BIOS mode the OS is loaded executing the instructions written
in a boot sector.  In the EFI mode the OS loader is a program stored in an
executable file, located in a directory of an EFI System partition (ESP).
The firmware of a computer can support one of these modes, or both.  In the
future the EFI mode will completely supersede the Legacy mode.
...
used file systems are vfat, ntfs, ext4, xfs, btrhs. This command lists all
file systems allowing to read data contained in a partition from Linux:
...
command, not to be typed.

1) Software update

A newly installed Slint contains software shipped in the ISO image.
...
Wen you are done issuing commands as 'root', press Ctrl+d or type exit to get
back your "regular user" status.

Alternatively the user registered during installation can type:
sudo <command>
then give the password for root
...
In case a kernel upgrade has been provided since the release of the ISO, type
 as root: upgrade-kernel

Also, consult the documentation provided for Slint, typing: slint-doc

2) Update of the configuration.

Here are the utilities that you can use to (re)configure your Slint system with
the default values set if you chose to install using 'auto', case occuring.
...
gtkkeyboardsetup
To configure the date, the time or the timezone: clocksetup and gtkcloksetup.
...
If KDE is installed to show or hide its applications in other desktop's menus:
show-kde-apps or hide-kde-apps.

Accessibility settings:
To choose a screen reader: speak-with
To start or not the orca screen reader on the desktop, as regular user: orca-on
or orca-off
...
====
+
Assuming that you installed Slint64-14.2.1 the output should be:
+
*Slint 14.2.1*
+
...
----
The files in /var/log/packages record information about the packages, mainly its content: the list of files it includes and where they are installed.
...
| calligra-l0n | localized Calligra office suite
